...
class P2PCnC(Signature):
    ...
    def on_complete(self):
        ...
        for tcp in self.get_results("network", {}).get("tcp", []):
            if (
                tcp["dport"] > 1023
                and tcp["dport"] not in self.ignoreports
                and tcp["dst"] not in servers
                and not tcp["dst"].startswith(
                    (
                        "0.",
                        "127.",
                        "169.254.",
        ...
        for udp in self.get_results("network", {}).get("udp", []):
            if (
                udp["dport"] > 1023
                and udp["dport"] not in self.ignoreports
                and udp["dst"] not in servers
                and not udp["dst"].startswith(
                    (
                        "0.",
                        "127.",
                        "169.254.",
